...
- **Authorization**: `Requester` (Must own the ticket), `IT Staff`, or `Admin`.
- **Response (200 OK)**:
  ```json
  {
    "id": 1,
...
### 2.3. Update Ticket Operational Fields (IT Staff / Admin)
...
- **Authorization**: `Requester` (Must own ticket), `IT Staff`, `Admin`.
- **Request Body**:
  ```json
  {
