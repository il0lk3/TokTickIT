...
- `GET /api/reference/categories`, `GET /api/reference/systems`, `GET /api/requesters`
...
- **AC-03 (Ownership):** Given Requester B is selected, when a Ticket belonging to Requester A is requested, then the Ticket data is not returned (403/404).
...
- **AC-06 (Soft Removal):** Given an active attachment, when the Requester soft-removes it, then it is marked as removed, remains in the UI as disabled/removed metadata, and the download API returns 404 or 410.
...
- Uploaded files will be stored locally in an `uploads` directory for simplicity during this lab.
