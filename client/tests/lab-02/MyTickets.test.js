...
describe("MyTickets Component", () => {
    ...
    it("shows loading state initially and fetches tickets", async () => {
        ...
        render(_jsx(TestWrapper, { requester: mockRequester, children: _jsx(MyTickets, { categories: mockCategories, onSelectTicket: vi.fn() }) }));
        expect(screen.getByText(/Loading tickets/i)).toBeInTheDocument();
        await waitFor(() => {
            expect(screen.getByText(/No tickets found/i)).toBeInTheDocument();
        });
        expect(api.getTickets).toHaveBeenCalledWith(expect.objectContaining({ search: "", categoryId: "", requestedPriority: "", status: "", page: 1 }), mockRequester.id);
    });
    ...
    it("debounces search input and triggers API fetch", async () => {
        ...
        await waitFor(() => {
            expect(api.getTickets).toHaveBeenCalledWith(expect.objectContaining({ search: "error" }), mockRequester.id);
        }, { timeout: 1000 });
    });
    it("handles sorting when header is clicked", async () => {
        ...
        // Initial fetch
        await waitFor(() => {
            expect(api.getTickets).toHaveBeenCalledWith(expect.objectContaining({ sortBy: "createdAt", sortOrder: "desc" }), mockRequester.id);
        });
        ...
        await waitFor(() => {
            expect(api.getTickets).toHaveBeenCalledWith(expect.objectContaining({ sortBy: "ticketNumber", sortOrder: "desc" }), mockRequester.id);
        });
        // Click again to toggle order
        fireEvent.click(ticketHeader);
        await waitFor(() => {
            expect(api.getTickets).toHaveBeenCalledWith(expect.objectContaining({ sortBy: "ticketNumber", sortOrder: "asc" }), mockRequester.id);
        });
    });
});
